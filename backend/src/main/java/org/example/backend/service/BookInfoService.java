...
public class BookInfoService {
    ...
    public BookInfo extractTitleAndAuthorFromPdf(MultipartFile file) throws IOException, InterruptedException {
        ...
        try (PdfReader reader = new PdfReader(new ByteArrayInputStream(file.getBytes()));
            ...
            String prompt = "Extract the title, author, and category of the book from the following text. The title should be the primary book title (e.g., 'Meditations'), not a series, edition, or collection name. Ignore metadata like 'Edited by' or 'Translated by'. The category should be a broad classification. Search for the book online and its category and return it classified. Respond with only the title, author, and category, each on a new line:\n\n" + extractedText;
            ...
            if (response.statusCode() != 200) {
                System.err.println("DeepSeek API error: Status " + response.statusCode() + ", Body: " + response.body());
                return new BookInfo("Error extracting title", "Error extracting author", "Unknown");
            }
            ...
            try {
                ...
                String title = lines.length > 0 ? lines[0].trim() : "Unknown Title";
                String author = lines.length > 1 ? lines[1].trim() : "Unknown Author";
                String category = lines.length > 2 ? lines[2].trim() : "Unknown";

                return new BookInfo(title, author, category);
            } catch (Exception e) {
                System.err.println("Error parsing DeepSeek response: " + e.getMessage());
                return new BookInfo("Error extracting title", "Error extracting author", "Unknown");
            }
